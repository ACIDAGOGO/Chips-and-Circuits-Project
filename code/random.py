...
def is_move_valid(wire: 'Wire', chip: 'Chip', desired_position: tuple[int, int]) -> bool:
    previous_position = wire.get_previous_position()
    grid_upperbounds = chip.get_grid_upperbounds()

    foreign_gate: bool == chip.check_for_illegal_gate(desired_position, wire.father)
    ...
    if desired_position == previous_position:
        ...
        tries_counter += 1
        return False
    elif desired_position[0] > grid_upperbounds[0] or desired_position[1] > grid_upperbounds[1] or desired_position[0] < 0 or desired_position[1] < 0:
        counter += 1
        tries_counter += 1
        return False
    elif foreign_gate:
